...
def main():
    ...
    with modus_client as client:
        ...
        if hr_float:
            ...
            logger.info(f"value={value}")
        hr_float = client.read_hr_float32(address=6, slave_number=1)
        logger.info(f"client.read_hr_float32: {hr_float}")
        ...
        if hr_string:
            ...
            logger.info(f"value={value}")
        hr_string = client.read_hr_string(address=16, string_length=10, slave_number=1)
        logger.info(f"client.read_hr_string = {hr_string}")
        ir_string = client.read_ir_string(address=16, string_length=10, slave_number=1)
        logger.info(f"client.read_ir_string = {ir_string}")
